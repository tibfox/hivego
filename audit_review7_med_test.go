...
func TestAuditReview7_HGM11_ShortBlockID(t *testing.T) {
	if got := blockNumberFromID(""); got != 0 {
		t.Errorf("HG-M11: empty id -> %d, want 0", got)
	}
	if got := blockNumberFromID("abc"); got != 0 {
		t.Errorf("HG-M11: short id -> %d, want 0", got)
	}
	if got := blockNumberFromID("0000000a1234567890"); got != 10 {
		t.Errorf("HG-M11: '0000000a...' -> %d, want 10", got)
	}
}
